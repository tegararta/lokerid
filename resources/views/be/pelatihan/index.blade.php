            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Tambahkan Pelatihan</h1>
                    ...
                    <div class="modal-body">
                        ...
                        <form action="{{ route('pelatihanbe.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="form-group">
                                <label>Hari pelatihan</label>
                                <input type="text" name="hari" class="form-control" placeholder="lokasi" required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>Ruangan</label>
                                <input type="text" name="ruangan" class="form-control" placeholder="lokasi" required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <textarea maxlength="120" type="text" name="deskripsi" class="form-control" placeholder="Masukkan deskripsi"
                                    required></textarea>
                            </div>
                    ...
                    <table class="table table-bordered table-hover" id="datatable">
                        ...
                        <tbody>
                            @if ($pelatihan->count() > 0)
                                @foreach ($pelatihan as $artikels)
                                    <tr>
                                        <th scope="row">{{ $loop->iteration }}</th>
                                        <td>{{ $artikels->judul }}</td>
                                        <td>{{ $artikels->nama }}</td>
                                        <td>{{ $artikels->pelatihan }}</td>
                                        <td>{{ $artikels->start }} - {{ $artikels->end }}</td>
                                        <td>{{ $artikels->lokasi }}</td>
                                        <td class="text-center">
                                            {{-- <div class="btn-group">
                                    <a data-id="" href="{{ route('pelatihanbe.show', $artikels->id) }}" class="btn btn-sm btn-info text-white show-modal mr-2"
                                        data-toggle="modal" data-target="#show_user">
                                        <i class="fas fa-fw fa-search"></i>
                                    </a>
                                </div> --}}
                                            <div class="btn-group">
                                                <a data-id="" href="{{ route('pelatihanbe.edit', $artikels->id) }}"
                                            ...
                                            <div class="btn-group">
                                                <form action="{{ route('pelatihanbe.destroy', $artikels->id) }}"
