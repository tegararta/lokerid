...
<div class="container mt-5">
	...
	<div class="d-flex justify-content-end mb-3">
		<a href="/daftarlokerbe"><button type="button" class="btn btn-secondary">
			Pendaftar Loker
		</button></a>
	</div>
					...
					<table class="table table-bordered table-hover" id="datatable">
						<thead>
							<tr>
								...
								<th scope="col">Pelatihan Pilihan</th>
								<th scope="col">Aksi</th>
							</tr>
						</thead>
						<tbody>
                            @if ($daftarpelatihan->count() > 0)
                            @foreach ($daftarpelatihan as $d )
							<tr>
								<td>{{ $loop->iteration }}</td>
								<td class="text-break">{{ $d->nama }}</td>
								<td class="text-break">{{ $d->nik }}</td>
								<td class="text-break">{{ $d->kelamin }}</td>
								<td class="text-break">{{ $d->nowa }}</td>
								<td class="text-break">{{ $d->pilihan }}</td>
								<td>
									<div class="btn-group">
										<a href="{{ route('daftarpelatihanbe.show',$d->id) }}" class="btn btn-sm btn-info text-white show-modal mr-2">
									...
									<div class="btn-group">
										<form action="{{ route('daftarpelatihanbe.destroy',$d->id) }}" onsubmit="return confirm('Yakin ingin menghapus?')" method="POST">
											@csrf
											@method('DELETE')
